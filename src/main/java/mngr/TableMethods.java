...
public class TableMethods {
    ...
    public static String toValidNameFile(String toValidString){
        ...
        for (char letter:toValidString.toCharArray()) {
            if (letter==' ' || letter == '.' || letter=='(' || letter==')' || letter=='_' || letter=='-' || Character.isLetterOrDigit(letter))
                validNameString+=letter;
        }
        if (validNameString.endsWith(".")) validNameString = validNameString.substring(0, validNameString.length() - 1);
